...
<head>
    ...
    <style>
        ...
        .table-responsive thead th {
            ...
            top: 0;
            z-index: 5;
            background-color: #f1f5fd; /* ensure header doesn't turn transparent */
        ...
        /* Metric columns should stay black (times are color-coded separately) */
        .hours-worked,
        .late-minutes,
        .undertime-minutes {
            color: #000;
        ...
        /* Scroll-only daily table: tuned to show ~15 rows then scroll */
...
<body>
    <div class="container-fluid py-4">
        <div class="row mb-4">
            <div class="col-12">
                ...
                <!-- helper upload description removed as requested -->
            </div>
        </div>
                            ...
                            <div class="d-flex align-items-center gap-2">
                                <div class="input-group input-group-sm" style="min-width: 260px;">
                                    <input
                                        ...
                                        id="dailySearchInput"
                                        class="form-control"
                                        placeholder="Search name, ID, department, date"
                                    />
    ...
    <!-- Employee Daily Time Records Modal (click from Monthly Summary) -->
                ...
                <div class="modal-body">
                    ...
                    <div class="table-responsive daily-table-scroll" style="max-height: 70vh;">
                        <table class="table table-hover" id="employeeDailyModalTable">
                            ...
                            <tbody id="employeeDailyModalBody">
                                <!-- populated via JS -->
                            </tbody>
                        </table>
                    </div>
    ...
    <!-- Date picker (calendar) for date filters -->
    ...
    <script>
        ...
        function parseUserDateToISO(input) {
            ...
            if (iso) return s;
            // Accept: MM-DD-YYYY, MM/DD/YYYY, MM-DD-YY, MM/DD/YY
        ...
        // Expected failed_logs.csv format: header "Employee ID","Department","Employee Name","Time","Date","Activity","Remarks"
        // Time = HH:MM:SS (24h), Date = MM/DD/YYYY, Activity = in/out. Column order detected from header.
        ...
        function parseCSV(csvContent) {
            ...
            // Raw logs often contain exact duplicates from sync retries; keep one copy only.
            employeeData = dedupeExactRecords(employeeData);
        }

        /** Parse date from failed_logs.csv format (MM/DD/YYYY) and return YYYY-MM-DD for grouping. */
        ...
        function processData() {
            ...
            for (const key in groupedData) {
                ...
                // Dedupe: treat punches within DEDUPE_WINDOW_MINUTES as same punch (double entry/lag); keep first only
                ...
                // Assign times (null = no punch in that window or unparseable time; no defaults used)
                if (morningIn || morningOut || afternoonIn || afternoonOut) {
                    ...
                    if (!monthlySummary[data.employeeId]) {
                     ...
                     // Missed logs frequency: count days with any missing punch (and total missing punches, if needed later)
                    ...
                    if (metrics.undertimeMinutes > 0) {
                     ...
                     // Absences: count Absent AM / Absent PM occurrences from status
            ...
            // Sort alphabetically by employee name, then by date
        ...
        function getScheduleForDepartment(department) {
            ...
            // Rules:
            // - Shop: 8:00 AM - 5:00 PM
            // - CT Print Shop / Ecotrade / Shop / Eco: 8:30 AM - 5:30 PM
            // - JCT: 9:00 AM - 6:00 PM
            if (d === 'shop') {
        ...
        function calculateMetrics(date, timeIn, breakOut, breakIn, timeOut, department) {
            ...
            const afternoonUndertimeEnd = createDateTime(date, addMinutesToTimeString(scheduleEnd, -1));
            ...
            // Absence rules (per your policy):
            // - Absent AM only if BOTH Time In and Break In are missing.
            // - Absent PM only if BOTH Break Out and Time Out are missing.
            // - Missing Break In and Break Out only (but has Time In + Time Out) is NOT absent.
            const absentAM = !actualStart && !actualBreakOut;
            const absentPM = !actualBreakIn && !actualEnd;
            // Half-day / incomplete: missing either Time In or Time Out (but not both)
            const hasTimeIn = !!actualStart;
            const hasTimeOut = !!actualEnd;
            const halfDayIncomplete = (hasTimeIn && !hasTimeOut) || (!hasTimeIn && hasTimeOut);

            // Always compute late/undertime from whatever punches exist (so duration is still reflected)
            let lateMinutes = 0;
            ...
            // Total hours only when we have both start and end
            let totalHours = '0.00';
            ...
            // Status: Absent based on missing AM/PM pairs; Half-day when only one of Time In/Out exists.
            // Otherwise Late/Undertime/Ontime (late/undertime duration is in table columns).
            let status;
    ...
    const lateFrequency = summary.totalLateFrequency;
                ...
                // Absences are counted in half-day units:
                // - Absent AM = 0.5
                // - Absent PM = 0.5
                // - Half Day (Incomplete Logs) = 0.5
        ...
        function openLetterModal(event, empId, type) {
            ...
            const text = buildLetterText(ctx);
            // WYSIWYG template editor (so blanks stay underlined even after typing)
        ...
        // Keep "fill" fields single-line and plain text so typing stays on the underline
        ...
        function buildLetterText(ctx) {
            ...
            const lines = [];
            // Header / inside address (placeholders you can customize)
            ...
            // IMPORTANT: use real newlines (not the literal characters "\n")
            return lines.join('\n');
        }
        ...
        const LETTER_MARGIN_MM = 25; // single source of truth for Print/PDF/Word margins
        ...
        async function downloadLetterPdf() {
            ...
            const MARGIN_PX = Math.round(96 * (LETTER_MARGIN_MM / 25.4)); // convert mm -> px at 96dpi
            const SCALE = 2;
            // Use pixel units so PDF page sizing matches html2canvas capture (helps match Word export layout).
            const pdf = new jsPDF({
                unit: 'px',
            ...
            // Build a renderable clone (must be on-screen; off-screen/hidden often produces blank PDFs).
            ...
            // Keep fully opaque for html2canvas capture; hide it by placing it behind the page.
            ...
            try {
                ...
                // Slice the canvas into page-sized chunks so margins are consistent on every page.
                ...
                for (let pageIndex = 0; pageIndex < totalSlices; pageIndex++) {
                    ...
                    const renderedHeight = sHeight / SCALE; // px in PDF space
        ...
        function formatLetterHtmlForPrint(text) {
            // Accept both real newlines and literal "\n" sequences (from copy/paste)
            const normalized = (text || '')
                .replace(/\\r\\n/g, '\\n')
                .replace(/\\n/g, '\\n')
                .replace(/\r\n/g, '\n')
                .trim();
            if (!normalized) return '<div class="block">(empty)</div>';

            const blocks = normalized.split(/\n\s*\n/);

            const isDateLine = (s) => /^\d{2}\/\d{2}\/\d{4}$/.test(s.trim());
            const isSubject = (s) => /^subject\s*:/i.test(s.trim());
            const isSalutation = (s) => /^dear\s+/i.test(s.trim());
            const isClosing = (s) => /^sincerely\s*,?$/i.test(s.trim());
            const isHeading = (s) => /^(details of concern\(s\)|a\.|b\.|received by:)/i.test(s.trim());
            const looksLikeList = (block) => block.split('\n').some(l => l.trim().startsWith('-'));
            const isMeta = (s) => /^(employee id:|department:)/i.test(s.trim());
            const isSignatureLine = (s) => /^_{10,}/.test(s.trim());
            ...
            return blocks.map((rawBlock, idx) => {
                ...
                const htmlLines = lines.map(l => `<div class="line">${escapeHtml(l)}</div>`).join('');

                // Classify block
                let cls = 'block';

                // Letterhead: first block usually contains company placeholders
                if (idx === 0 && first.startsWith('[')) cls += ' letterhead';
                else if (isDateLine(first)) cls += ' date';
                else if (isSubject(first)) cls += ' subject';
                else if (isSalutation(first)) cls += ' salutation';
                else if (isClosing(first)) cls += ' closing';
                else if (isHeading(first)) cls += ' heading';
                else if (isSignatureLine(first)) cls += ' signature-line';
                else if (looksLikeList(block)) cls += ' list';
                else if (isMeta(first) || (lines.length >= 2 && (lines[1] || '').toLowerCase().startsWith('employee id:'))) cls += ' meta';
                else {
                    // Default paragraph indentation for normal body text blocks
                    const isLongSentence = block.replace(/\s+/g, ' ').length >= 60;
                    if (isLongSentence) cls += ' para';
                }

                return `<div class="${cls}">${htmlLines}</div>`;
            }).join('');
        }
        ...
        function getLetterExportCss() {
            // Shared content CSS (typography/layout). Page margins are handled by the exporter (@page/PDF placement).
            return `
                .letter {
                    font-family: "Times New Roman", Times, serif;
                    font-size: 12pt;
        ...
        function normalizeTimeTo24(timeStr) {
            ...
            // Strip optional milliseconds (e.g. 12:03:17.000 or 12:03:17.123456)
        ...
        /** Format date for display as MM/DD/YYYY (accepts YYYY-MM-DD or MM/DD/YYYY). */
        ...
        /**
         * Keep only one punch per cluster: if two punches are within DEDUPE_WINDOW_MINUTES,
         * treat as double entry/lag and keep the first. Prevents near-duplicate logs from
         * matching into different columns (e.g. two "in" 30s apart → one Time In only).
         */
        ...
        /**
         * Infer activity type from time ranges and order, ignoring CSV activity column.
         * Time In: 1st punch in 00:00–12:00. Break In: 1st in 08:30–12:30 (not Time In).
         * Break Out: 1st punch in 12:45–17:30. Time Out: last punch in 13:00–23:59.
         */
        function inferActivitiesByTimeRange(activities) {
            const winMorningInEnd = timeToMinutes(BREAK_OUT_TIME);           // 12:00
            const winBreakOutStart = timeToMinutes(WORK_START_TIME);          // 08:30
            const winBreakOutEnd = timeToMinutes(BREAK_OUT_LATEST);
            const afternoonStart = timeToMinutes(BREAK_IN_EARLIEST);         // 12:45
            const earlyBreakInStart = timeToMinutes(EARLY_BREAK_IN_START);   // 12:31
            ...
            // Morning: Time In = first punch in [0, 12:00]
            ...
            // Break In = first punch in [08:30, 12:44:59] that isn't Time In
            ...
            // Afternoon: Break Out = first punch in [12:45, end-of-day]
            const afternoonInEnd = timeToMinutes(WORK_END_TIME);
            const afternoonOutStart = timeToMinutes(BREAK_IN_TIME); // 13:00
            ...
            if (afternoonInEnd !== null && afternoonOutStart !== null) {
                ...
                // Time Out must be a distinct later punch (do not reuse Break Out as Time Out)
        ...
        function selectFirstInWindow(activities, activityType, windowStart, windowEnd) {
            ...
            return activities.find(item =>
                ...
                item.minutes !== null &&
                item.minutes >= startMinutes &&
                item.minutes <= endMinutes
            ) || null;
        }

        function selectLastInWindow(activities, activityType, windowStart, windowEnd) {
            const startMinutes = timeToMinutes(windowStart);
            const endMinutes = timeToMinutes(windowEnd);
            if (startMinutes === null || endMinutes === null) return null;
        ...
        // Render initial empty state (placeholders) on page load
        displayResults();
        // No auto sample load: process only user-uploaded CSV to avoid runtime errors.
    </script>
</body>
</html> 